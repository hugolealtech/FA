/**
 * ============================================================================
 * FUNCTUS v8.2.3 - JUSTICE UPDATE (Vavilov Patch + Smart Traffic Lights)
 * ============================================================================
 */
...
const Store = {
    ...
    data: {
        ...
        categories: [
            ...
            "Veículo: Combustível", "Veículo: Manutenção", "Reembolso/Estorno" 
        ],
...
const Ledger = {
        ...
        Store.data.expenses.forEach(exp => {
            ...
            if (isApplicable) {
                ...
                if(matchesFilter) {
                    ...
                    if(exp.type === 'estorno' && val > 0) val = val * -1;
                    if (exp.type === 'recorrente' && exp.isVariable) {
                        if (exp.variations && exp.variations[targetYM]) { val = parseFloat(exp.variations[targetYM]); } else { isEstimate = true; }
                    }
                    ...
                    else if(days <= 5) { status = 'status-warning'; }
                    if(exp.type === 'estorno') status = 'status-refund';
    ...
    getDebtEndDate(exp) {
        ...
        if (exp.type === 'avista' || exp.type === 'cartao_avista' || exp.type === 'estorno') return Utils.formatDate(exp.date);
...
const App = {
    ...
    renderPlanner(el) {
        ...
        const cardHtml = activeCards.map(c => {
            ...
            // --- LÓGICA DE CORES (TRAFFIC LIGHT) ---
            const today = new Date(); today.setHours(0,0,0,0);
            const dueObj = new Date(d.getFullYear(), d.getMonth(), c.dueDay);
            const diffDays = Utils.diffDays(dueObj, today);
            
            let urgencyColor = 'var(--success)'; // Padrão Verde (Longe)
            let badgeText = `FATURA ${Utils.formatMonth(d)}`;

            if(ledger.totalDue > 0) {
                if (diffDays <= 3) { // 3 dias ou menos (ou vencido)
                    urgencyColor = 'var(--danger)'; 
                    badgeText = 'VENCE LOGO / VENCIDA';
                } else if (diffDays <= 10) { // Entre 4 e 10 dias
                    urgencyColor = 'var(--accent)'; 
                    badgeText = 'ATENÇÃO AO VENCIMENTO';
                }
            } else {
                 urgencyColor = 'var(--secondary)'; // Zerado/Pago
                 badgeText = 'FATURA ZERADA';
            }
            // ---------------------------------------
            ...
            <div class="planner-card-container">
                <div class="credit-card-v3 ${c.skin || 'skin-black'}" style="height:160px; padding:20px; box-shadow:0 5px 15px rgba(0,0,0,0.2)">
                     <div class="card-status-badge" style="background:${urgencyColor}; transition: background 0.3s">${badgeText}</div>
        ...
        // --- LÓGICA DE CORES PARA BOLETOS ---
        let bolColor = 'var(--success)';
        let bolText = 'CONTAS & BOLETOS';
        if(boletoLedger.totalDue > 0) {
            // Encontra a data mais próxima que ainda não foi paga
            const today = new Date(); today.setHours(0,0,0,0);
            let minDiff = 999;
            boletoLedger.items.forEach(item => {
                if(!item.isPaid) {
                    const diff = Utils.diffDays(item.dueDate, today);
                    if(diff < minDiff) minDiff = diff;
                }
            });
            if(minDiff <= 3) { bolColor = 'var(--danger)'; bolText = 'BOLETOS URGENTES'; }
            else if(minDiff <= 10) { bolColor = 'var(--accent)'; bolText = 'BOLETOS PRÓXIMOS'; }
        } else if(boletoLedger.total > 0 && boletoLedger.totalDue === 0) {
             bolColor = 'var(--secondary)'; // Tudo pago
        }
        // -----------------------------------
        ...
        const boletoHtml = `
            <div class="planner-card-container">
                <div class="credit-card-v3 skin-boleto" style="height:160px; padding:20px; box-shadow:0 5px 15px rgba(0,0,0,0.2)">
                     <div class="card-status-badge" style="background:${bolColor}">${bolText}</div>
        ...
        const totalHtml = `
             <div class="planner-card-container">
                ...
                <div class="planner-card-actions">
                    <button class="btn btn-dark btn-sm" onclick="app.payAllMonth()">Pagar Tudo</button>
                    <button class="btn btn-secondary btn-sm" onclick="app.payCashOnly()" title="Pagar somente compras à vista">Quitar À Vista</button>
                </div>
            </div>`;
            ...
            <div class="glass-panel">
                ...
                <div>
                    ${filteredLedger.items.map(i => {
                        ...
                        if(!i.isPaid && i.type !== 'estorno' && i.val > 0) {
                             manageBtn += `<button class="btn btn-ghost" style="color:var(--danger)" onclick="app.refundItem('${i.id}')" title="Estornar/Cancelar Compra"><i class="fa-solid fa-rotate-left"></i></button>`;
                        }
                        
                        // LÓGICA DE VISUALIZAÇÃO DE ITENS INDIVIDUAIS
                        // Itens de cartão ficam "Verdes/Neutros" na lista (pois a urgência é do cartão).
                        // Itens de Boleto (sem cardId) respeitam a cor de perigo calculada.
                        let rowClass = i.status;
                        if(i.cardId && !i.isPaid) {
                             rowClass = 'status-safe'; 
                        }
                        
                        let rowStyle = '';
                        if(i.type === 'estorno') {
                            rowStyle = 'border-left: 3px solid #3b82f6; background: rgba(59, 130, 246, 0.05);';
                        }
                        return `
                        <div class="planner-item ${rowClass}" style="${rowStyle}">
                            <div>
                            ...
                            <div style="text-align:right; display:flex; gap:5px; align-items:center;">
                                <div style="font-weight:700; margin-right:10px; ${i.val < 0 ? 'color:#3b82f6' : ''}">${i.isEstimate ? '~' : ''}${Utils.formatCurrency(i.val)}</div>
                                ${manageBtn}
                                ${actionBtn}
                            </div>
                        </div>`;
    ...
    toggleType(val) {
        ...
        const isRefund = val === 'estorno';
        const showCardGroup = isCard || isRecurrent || isRefund;
        ...
        if (isRecurrent) {
            ...
            dateLabel.innerText = "Data do 1º Vencimento";
             ...
             dateLabel.innerText = "Data do Estorno";
        } else {
    ...
    addExpense(e) {
        ...
        let finalValue = parseFloat(f.value.value);
        if(type === 'estorno' && finalValue > 0) finalValue = finalValue * -1;
    ...
    refundItem(id) {
        const item = Store.data.expenses.find(e => e.id === id);
        if(!item) return;
        if(!confirm(`Deseja criar um estorno para "${item.description}"?\n\nIsso criará um crédito na fatura.`)) return;
        const totalInst = parseInt(item.installments) || 1;
        const paidCount = item.paidPeriods ? item.paidPeriods.length : 0;
        let refundValue = 0;
        if(totalInst > 1) {
             const parcelVal = parseFloat(item.value) / totalInst;
             const remaining = totalInst - paidCount;
             refundValue = parcelVal * remaining;
        } else {
             refundValue = parseFloat(item.value);
        }
        Store.data.expenses.push({
            id: Utils.uuid(),
        ...
        Store.save();
        alert("Estorno lançado com sucesso! O crédito aparecerá na fatura.");
        this.render();
    },

    payCashOnly() {
        if(!confirm("Deseja marcar como PAGO todas as compras à vista deste mês?\nIsso manterá as parcelas em aberto.")) return;
        const ledger = Ledger.getMonthlyLedger(this.dashDate.getFullYear(), this.dashDate.getMonth()+1, 'ALL');
        let count = 0;
        ledger.items.forEach(item => {
            if (!item.isPaid && (item.type === 'avista' || item.type === 'cartao_avista')) {
                const exp = Store.data.expenses.find(e => e.id === item.id);
                if(exp) { 
                    if(!exp.paidPeriods) exp.paidPeriods = []; 
                    if(!exp.paidPeriods.includes(item.targetYM)) {
                        exp.paidPeriods.push(item.targetYM);
                        count++;
                    }
                }
            }
        ...
        Store.save();
        alert(`${count} compras à vista foram quitadas.`);
        this.render();
    },
    ...
    checkDuplicates() {
        ...
        const seen = new Map();
        Store.data.expenses.forEach(exp => {
            const valKey = parseFloat(exp.value).toFixed(2);
            const dateKey = exp.date;
            const key = `${valKey}|${dateKey}`;
    ...
    showDuplicateResolver() {
        const dups = this.checkDuplicates();
        if(dups.length === 0) return alert("Nenhuma duplicidade encontrada.");
        let htmlList = dups.map((group, gIdx) => `
            <div style="background:rgba(0,0,0,0.05); padding:10px; border-radius:8px; margin-bottom:10px;">
                <div style="font-weight:bold; margin-bottom:5px; color:var(--danger)">Grupo Suspeito #${gIdx+1} - ${Utils.formatCurrency(group[0].value)}</div>
                ${group.map(item => `
                    <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px solid var(--glass-border); padding:5px 0">
                        <div style="font-size:0.85rem">
                            <strong>${item.description}</strong><br>
                            ${Utils.formatDate(item.date)} • ${item.category}
                        </div>
                        <button class="btn btn-sm btn-ghost" style="color:red" onclick="app.deleteExpense('${item.id}', true)">
        ...
        <div class="modal-overlay" id="dupModal">
            <div class="modal-content">
                <h2><i class="fa-solid fa-magnifying-glass-chart"></i> Resolver Duplicidades</h2>
                <p>O sistema encontrou itens com mesmo valor e data. Exclua os incorretos.</p>
                <div style="max-height:400px; overflow-y:auto">
                    ${htmlList}
                </div>
                <button class="btn btn-primary" onclick="document.getElementById('dupModal').remove(); app.render()">Fechar</button>
            </div>
        </div>`;
        document.body.insertAdjacentHTML('beforeend', modalHTML);
    },

    deleteExpense(id, fromModal = false) {
    ...
    processPastedText() {
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            if (instStr) {
                ...
                desc = `${desc} (${cleanInst})`; 
                const parts = cleanInst.split('/');
                if(parts.length === 2) {
                    installments = parseInt(parts[1]); 
                }
            }
            let value;
            if (valStr.includes(',')) {
    ...
    confirmImport() {
        ...
        this.importedItems.forEach((item, idx) => {
            ...
            const isParcelado = item.inst > 1;
            const finalType = isParcelado ? 'parcelado' : 'cartao_avista';
            Store.data.expenses.push({
                id: Utils.uuid(),
